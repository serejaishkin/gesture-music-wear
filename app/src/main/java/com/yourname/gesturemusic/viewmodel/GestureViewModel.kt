...
class GestureViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private val gestureReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent?) {
            intent?.getStringExtra("gesture")?.let {
    ...
    init {
        context.registerReceiver(gestureReceiver, IntentFilter("com.yourname.gesturemusic.GESTURE_DETECTED"), Context.RECEIVER_NOT_EXPORTED)
        sendSensitivityUpdate()
    }

    fun startService() {
        context.startForegroundService(Intent(context, GestureMusicService::class.java).apply { action = GestureMusicService.ACTION_START })
        _isRunning.value = true
    }

    fun stopService() {
        context.startService(Intent(context, GestureMusicService::class.java).apply { action = GestureMusicService.ACTION_STOP })
        _isRunning.value = false
    }

    fun updateAngleThreshold(value: Float) { _angleThreshold.value = value }
    fun updatePinchThreshold(value: Float) { _pinchThreshold.value = value }
    fun updateMinDuration(value: Long) { _minDuration.value = value }
    fun updateMaxDuration(value: Long) { _maxDuration.value = value }
    fun updateGestureCooldown(value: Long) { _gestureCooldown.value = value }
    fun updateLeftHand(value: Boolean) { _leftHand.value = value }
