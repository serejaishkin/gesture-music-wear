...
import com.yourname.gesturemusic.gesture.GestureDetectionStrategy
import com.yourname.gesturemusic.gesture.GestureTrainer
import com.yourname.gesturemusic.gesture.GestureType
import com.yourname.gesturemusic.gesture.RawSensorStrategy
import com.yourname.gesturemusic.gesture.SamsungGestureStrategy
...
class GestureMusicService : Service(), SensorEventListener, GestureDetectionStrategy.GestureListener {
    companion object {
        ...
        const val ACTION_STRATEGY_INFO="com.yourname.gesturemusic.STRATEGY_INFO"; const val EXTRA_STRATEGY_NAME="strategy_name"
    }
    private lateinit var sensorManager:SensorManager; private lateinit var mediaControllerManager:MediaControllerManager
    private lateinit var gestureTrainer:GestureTrainer; private lateinit var armingManager:GestureArmingManager
    ...
    private var activeStrategy: GestureDetectionStrategy? = null
    ...
    override fun onCreate(){
        ...
        wakeLock=pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK,"GestureMusic::WakeLock")
        wakeLock.setReferenceCounted(false)
        vibrator=getSystemService(Context.VIBRATOR_SERVICE) as Vibrator
        notificationManager=getSystemService(NotificationManager::class.java)
        mediaControllerManager.setStateListener{playing->isMusicPlaying=playing}
        val f=IntentFilter().apply{addAction(Intent.ACTION_SCREEN_OFF);addAction(Intent.ACTION_SCREEN_ON)}
        if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.TIRAMISU)registerReceiver(screenStateReceiver,f,Context.RECEIVER_EXPORTED)else registerReceiver(screenStateReceiver,f)
    ...
    override fun onStartCommand(intent:Intent?,flags:Int,startId:Int):Int{
        when(intent?.action){
            ...
            ACTION_UPDATE_SENSITIVITY->if(isRunning)updateSensitivity(intent)
            ACTION_START_TRAINING->startTraining(intent)
            ACTION_STOP_TRAINING->cancelTraining()
    ...
    // --- Sensor callbacks (used in training mode & raw sensor fallback) ---
    override fun onSensorChanged(event:SensorEvent?){
        event?:return;val timestamp=SystemClock.elapsedRealtime()
        when(event.sensor.type){
            Sensor.TYPE_GYROSCOPE->{lastGyroX=event.values[0];lastGyroY=event.values[1];lastGyroZ=event.values[2]}
            Sensor.TYPE_LINEAR_ACCELERATION->{
                lastLinAccX=event.values[0];lastLinAccY=event.values[1];lastLinAccZ=event.values[2]
                if(isTrainingMode){
                    val e=gestureTrainer.addSample(lastGyroX,lastGyroY,lastGyroZ,lastLinAccX,lastLinAccY,lastLinAccZ)
                    val p=gestureTrainer.getRecordingProgress()
                    if(p!=lastTrainingProgress){lastTrainingProgress=p;sendTrainingProgress(p,gestureTrainer.getTrainingRepetitionCount(),false,false)}
                    if(e==GestureTrainer.TrainingEvent.REPETITION_ACCEPTED)onTrainingRepetitionAccepted()
                }else if(activeStrategy is RawSensorStrategy){
                    // RawSensorStrategy handles its own processing via its own SensorEventListener
                }else{
                    // Samsung strategy active, but we still get raw sensor data for DTW trainer
    ...
    private fun processGestures(timestamp:Long){
        ...
        val gesture=effectiveLearned?:run{
            val w=WristRotationDetector().process(timestamp,lastGyroX,lastGyroY,lastGyroZ,lastLinAccX,lastLinAccY,lastLinAccZ)
            val p=com.yourname.gesturemusic.gesture.DoublePinchDetector().process(timestamp,lastGyroX,lastGyroY,lastGyroZ,lastLinAccX,lastLinAccY,lastLinAccZ)
            w?:p
        }
        gesture?.let{processGestureResult(it,timestamp)}
    }

    private fun processGestureResult(gesture: GestureType, timestamp: Long) {
    ...
    // --- Strategy selection ---
    private fun selectStrategy(): GestureDetectionStrategy {
        val samsung = SamsungGestureStrategy()
        if (samsung.isAvailable(this)) {
            Log.d(TAG, "Using Samsung Gesture API (Galaxy Watch 4+)")
            return samsung
        }
        Log.d(TAG, "Using RawSensor fallback (universal)")
        return RawSensorStrategy()
    }

    // --- Training (always uses raw sensors) ---
    ...
    private fun onTrainingRepetitionAccepted(){
        ...
        if(count>=gestureTrainer.getRequiredRepetitions()){
            ...
            sendTrainingProgress(100,count,true,false)
            Thread{
                val success=try{gestureTrainer.saveTraining(type)}catch(e:Exception){Log.e(TAG,"saveTraining failed",e);false}
    ...
    private fun startGestureDetection(){
        ...
        // Select and start the best detection strategy
        val strategy = selectStrategy()
        activeStrategy = strategy
        strategy.start(this, this)
        sendStrategyInfo(strategy.name)

        if (strategy is RawSensorStrategy) {
            // RawSensorStrategy registers its own sensors, but we also need
            // sensors for DTW training — register our own too
            ensureSensorsRegistered()
        }
    ...
    private fun updateSensitivity(intent:Intent){
        ...
        activeStrategy?.updateSettings(angle,pinch,cooldown,left,minDur,maxDur)
    }
    ...
    private fun sendStrategyInfo(name:String)=sendBroadcast(Intent(ACTION_STRATEGY_INFO).apply{putExtra(EXTRA_STRATEGY_NAME,name)})
